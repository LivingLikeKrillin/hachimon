...
| 호스팅·보안 | **토큰-게이트**(사적 감상) — capture의 **Bearer 토큰 게이트 패턴 차용**한 Cloudflare Worker로 notes.json 서빙(읽기 위주, capture와 흐름은 다름) |
| 스케줄 | **Leitner류 stage**(2d→5d→12d→30d→90d…). 신호가 stage 전이(SM-2 등급 아님) |
| 콘텐츠 선택 | frontmatter **`captureId:` 보유** vault 노트 전체(= anchor가 안착시킨 모든 노트). kind는 anchor 규칙으로 파생(`kind:` 값 있으면 그것, 없으면 capture). `recall: false` opt-out은 후속 |
...
| `worker: src/relay.ts` | 읽기 서빙 `GET /notes`(Bearer 검증)→notes.json; 쓰기 업로드 `PUT /notes`(Bearer)→R2/KV 저장. 두 핸들러 분리(읽기 경로=폰, 쓰기 경로=publish CLI 독립 테스트·인증) | R2/KV | 얇음(miniflare) |
...
- **선택:** vault 노트 중 frontmatter **`captureId:` 보유분** = anchor가 안착시킨 모든 노트(reflection·memo·capture 3 kind 전부 `captureId`를 씀). Hachimon 기술노트는 `captureId` 없어 자동 제외.
  - **주의(실측 반영):** capture-kind 노트는 `kind:` 라인을 **안 쓴다**(anchor `assembleRaw/Transformed`는 `type:`만 씀; anchor `noteKind`는 *kind 부재 = capture*로 본다). 따라서 `kind:` 존재로 거르면 capture 노트가 전부 누락된다 → **선택 술어는 `captureId` 존재**여야 한다. `kind`는 파생: `frontmatter.kind` 값이 있으면 그것(reflection/memo), 없으면 `capture`(anchor `noteKind` 규칙 동일).
- **publish CLI**(`scripts/publish-notes.ts`, capture `sync-captures`·Hachimon `parse-vault` CLI 패턴 차용): vault 디렉토리 스캔 → `captureId` 보유 노트의 frontmatter+본문 파싱 → `FeedNote` 조립. 파서는 **임의의 추가 frontmatter 키에 관대**해야 한다(reflection의 `medium/work/workKey/progress`, capture의 `type/imageExt` 등은 무시). 본문은 frontmatter·임베드(`![[...]]`) 라인 제거한 prose; capture는 `## 핵심`/`## 원문` 섹션이 prose가 된다. `title`은 frontmatter `title`(없으면 `source` 또는 첫 줄 폴백). `sourceHash`=본문 해시 → `notes.json` → **Worker 업로드 핸들러**(토큰)로 PUT. 수동 + 윈도우 작업 스케줄러(capture sync 패턴).
- `id`=frontmatter `captureId`. 안정 키라 폰 recall 상태가 재배포 간 보존. (`captureId` 없는 손수 작성 노트는 v1 비대상 — 경로-해시 폴백은 파일 rename 시 재설정되므로 안 씀.)
...
안착 prose = 사적 감상이라 **공개 정적 URL 금지**. capture의 **Bearer 토큰 게이트 idiom 차용**(흐름은 다름 — capture는 폰이 POST·소비-삭제, echo는 CLI가 쓰고 폰이 읽기만):
...
| notes.json 파싱 | 손상/잘림(flaky fetch) | fetch 실패와 동일 취급 — 기존 캐시 유지, merge 안 함 |
...
- [ ] publish CLI가 vault의 `captureId` 보유 안착 노트(reflection·memo·capture 3 kind 전부)를 notes.json으로 조립해 토큰 게이트 호스팅에 배포.
...
- 콘텐츠 선택은 **`captureId` frontmatter 기준**(kind 아님 — capture 노트는 `kind:`를 안 쓰므로). anchor 승격은 `stripStagingFrontmatter`가 `status`만 제거 → `captureId`·`kind`(reflection/memo)·`type`(capture)·`title` 등 보존. 그래서 `captureId`로 거르면 세 kind 모두 잡히고, kind는 `kind:` 값(있으면)·없으면 capture로 파생.
- §6 `selectDue`의 serendipity는 **선택적**(DoD 불요건) — seed 뒤에 두어 시간 빠듯하면 순수 결정적(overdue·오래된 우선)으로 출하 가능.
