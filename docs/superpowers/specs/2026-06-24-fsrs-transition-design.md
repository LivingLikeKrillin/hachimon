...
| 평가 매핑 | Again(0)→1, Hard(2)→2, Good(4)→3, Easy(5)→4 |
...
기존 `Quality = 0 | 2 | 4 | 5` 유지 (UI 버튼은 그대로). `fsrs.ts` 내부에서 ts-fsrs `Rating`(1~4)으로 매핑한다.
...
| `previewIntervals` | `(schedule: Schedule, now?: Date) => Record<Quality, number>` | `f.repeat()`로 4개 평가 각각의 다음 간격(scheduledDays, 일)을 반환. `getNextInterval` 대체. |
...
- `tierAccuracy` 등에서 `easeFactor`/`repetitions`를 직접 참조하던 부분을 FSRS 필드로 교체(해당 통계가 ef/rep 기반이면 stability/state 기반으로 재정의하거나 제거).
- `getDueCards` / `getDueCount` / `getNewCardCount`의 `nextReviewAt <= now`, `lastReviewedAt !== null` 조건은 필드명을 유지하므로 **변경 없음**.
...
- `merge.ts`, `useReviewSession.ts`의 `from './sm2'` / `from '@/lib/sm2'` import를 `fsrs`로 교체.
...
| `src/lib/merge.ts` | import 교체 |
| `src/lib/data.ts` | 마스터 정의, ef/rep 참조 교체 |
| `src/lib/backup.ts` | 스키마 버전 갱신 |
| `src/hooks/useReviewSession.ts` | import 교체, previewIntervals 사용 |
| `src/hooks/useSettings.ts` | requestRetention 추가, EF 제거 |
| `src/pages/Settings.tsx` | retention 슬라이더로 교체 |
| `src/App.tsx` (init) | `migrateToFsrs()` 호출 추가 |
