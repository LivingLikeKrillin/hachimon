...
- 스크립트는 ESM으로 작성한다(`package.json`이 `"type": "module"`; `import` 사용, `require` 금지).
- **주 설계 — 전용 `tsconfig.scripts.json`:** `tsconfig.app.json`이 이미 `include: ["src"]`로 `obsidian.ts`를 소유하므로, 이를 node 프로젝트에도 끌어오면 `tsc -b`에서 "file in multiple projects" 충돌이 유력하다. 따라서 `scripts/`는 **별도 `tsconfig.scripts.json`**으로 분리하고 root `tsconfig.json`의 `references`에 추가한다. 구성:
  - `include: ["scripts/**/*.ts"]`, `types: ["node"]`, `composite: true`(다른 프로젝트와 동일), `tsBuildInfoFile` 지정.
  - `baseUrl: "."` + `paths: { "@/*": ["./src/*"] }` — `obsidian.ts`의 type-only `@/types` import 해석용.
  - `obsidian.ts`는 `tsconfig.app.json`이 소유하고, scripts 프로젝트는 이를 type-only로만 참조하므로 소유권 충돌이 없다.
- 테스트 파일(`scripts/parse-vault.test.ts`)도 포함되지만, `obsidian.test.ts`와 동일하게 `import { describe, it, expect } from 'vitest'`로 명시 import하므로 vitest 전역 타입 없이도 `tsc`가 통과한다.
- 결과적으로 `tsc -b`(및 `npm run build`)가 스크립트를 타입체크한다.
- (대안) `tsconfig.node.json` include에 `scripts/**/*.ts`를 더하고 거기에 `@/*` 별칭을 추가하는 방법도 가능하나, 위 다중프로젝트 충돌 위험 때문에 전용 tsconfig를 1순위로 한다.
...
| `tsconfig.scripts.json` | **신규** — scripts 전용(`@/*` 별칭, types node, composite); root `tsconfig.json` references에 추가 |
| `tsconfig.json` | root references에 scripts 프로젝트 추가 |
