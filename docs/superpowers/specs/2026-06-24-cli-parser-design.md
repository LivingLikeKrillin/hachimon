...
- **전용 `tsconfig.scripts.json`을 독립 실행(`tsc -p`)으로 체크:** `tsconfig.app.json`이 이미 `include: ["src"]`로 `obsidian.ts`를 소유하므로, 이를 composite 참조 그래프(root `tsc -b`)에 끌어오면 "file in multiple projects" 충돌이 유력하다. 따라서 `tsconfig.scripts.json`은 **root `references`에 넣지 않고** `tsc -p tsconfig.scripts.json`으로 독립 타입체크한다. `tsc -p`는 빌드모드가 아니라 단일 프로그램 체크라, import된 `obsidian.ts`를 함께 검사만 할 뿐 소유권 충돌이 없다. 구성:
  - `include: ["scripts/**/*.ts"]`, `types: ["node"]`, `noEmit: true` (composite·root references 불필요). `tsconfig.node.json`의 옵션을 본뜬다.
...
- 드리프트 방지: `npm run build`를 `tsc -b && tsc -p tsconfig.scripts.json && vite build`로 확장해 스크립트 타입체크가 빌드에 항상 포함되게 한다. 편의용 `typecheck:scripts` 스크립트도 추가.
...
| `tsconfig.scripts.json` | **신규** — scripts 전용(`@/*` 별칭, types node, noEmit); `tsc -p`로 독립 체크(root references 미추가) |
