...
import { assembleNote, toSlug, uniqueSlug, countParsedCards } from '../src/lib/forge/assemble.ts';
...
async function run(args: InboxArgs): Promise<RunResult> {
  ...
  for (const file of inboxFiles) {
    try {
      ...
      const cardCount = countParsedCards(md, created);
      if (cardCount === 0) {
        console.warn(`⚠ 보류(카드 0): ${file.name} — 인박스 원본 보존`);
        held++;
        continue;
      }
      ...
      if (args.dryRun) {
        console.log(`\n— [dry-run] ${file.name} → ${outPath} (${cardCount} cards)\n${md}`);
      } else {
...
**핵심 게이트:** `assemble.test.ts`의 parseVault 라운드트립(조립 → 실제 파서 → 카드 ≥1). 이것이 "draft가 vault에서 실제 카드로 빌드되는가"를 단일 진실원천으로 보증한다. 네트워크/Claude 호출은 단위 테스트하지 않고 `--dry-run` 수동 검증으로 대체한다(스펙 §9).
