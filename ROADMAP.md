...
- [ ] Home "면접 훈련 모드" → InterviewFilter 전환
...
## Phase 2 — 면접 훈련 모드 (v0.2)

맞춤형 복습 세션을 구성할 수 있게 한다.

### 2-1. InterviewFilter 구현

- [ ] 덱 트리 다중 선택 (체크박스)
- [ ] 티어 필터 칩 (Foundation / Mechanism / Diagnosis 토글)
- [ ] 세션 크기 슬라이더 (5~30장)
- [ ] "시작" 버튼 → 선택 조건으로 카드 필터링 → ReviewSession 진입
- [ ] 선택 없음 방지 (최소 1개 덱, 1개 티어)
...
2. **Phase 2는 1 직후.** 면접 훈련이 핵심 시나리오다.
