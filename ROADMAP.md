...
- [~] `src/lib/syntax.ts` 커스텀 토크나이저 — **미채택**. `highlight.js` + `rehype-highlight`로 대체(Java/Kotlin·SQL·YAML 등 광범위 언어 기본 지원, 유지보수 부담 없음).
- [x] One Dark Pro 계열 테마 적용 — `src/index.css` `.hljs-*` 규칙 (muted gold accent에 맞춤).
- [x] 코드블록에 줄 번호 + 언어 라벨 표시 — `src/components/shared/CodeBlock.tsx`(react-markdown `pre` override) + `src/lib/codeblock.ts`(언어/줄 수 추출, 테스트 포함).
...
### 5-5. 노트→카드 파서 강화 (완료 — 멀티라인 답변)

- [x] 멀티라인 답변 — `질문?::` 답변을 다음 카드/티어/헤딩/EOF까지 누적(코드펜스·인라인코드 인식). 앱이 렌더링하도록 설계된 코드블록·문단·리스트 답변을 실제 노트에서 도달 가능하게.
- [x] in-app·CLI·플러그인 공통(`parseVault` 단일 진실원천), 기존 단일 줄 하위호환.
